...
- `frontend/` — Next.js 16 App Router app (not `apps/web/` as originally planned). Full app shipped: auth flow, sidebar shell, home dashboard, the sketch-upload / camera-capture create flow, the form editor, the drafts / trash / published / responses workspaces, the public respondent page (`/f/[slug]`), an in-app product tour, and light/dark theming.
...
Four user tables plus one view: `profiles`, `forms`, `form_responses`, `generation_jobs`, and the `public_forms` view. Key invariants set up across the migrations in `supabase/migrations/`:
...
- **Soft delete / trash**: `forms.deleted_at` (nullable `timestamptz`) marks a draft as trashed. The `forms_guard_soft_delete` BEFORE UPDATE trigger enforces that only `draft` forms can be trashed and that restore is allowed only within a 1-day retention window. `purge_expired_deleted_forms()` (`SECURITY DEFINER`) hard-deletes expired rows *and* their sketch Storage objects; a `pg_cron` job runs it hourly. The frontend mirrors the window as `TRASH_RETENTION_MS` in `lib/data/forms.ts`. List queries must filter `deleted_at IS NULL` to exclude trashed drafts.
- **Multi-sketch create**: `forms.sketch_paths text[]` holds every sketch image used to generate a form; `forms.sketch_path` remains the primary (first) path and serves as the thumbnail for existing UI.
- **Product tour state**: `profiles.tour_completed_at` — `NULL` auto-triggers the in-app tour on next session; finishing or skipping it stamps the column. Existing rows were backfilled at migration time so only genuinely new sign-ups see the tour automatically.
...
- **Route groups**: `src/app/(auth)/` for login/signup/forgot-password; `src/app/(app)/` for the authed shell (sidebar layout, home, and the `forms/*` subtree — `create`, `create/blank`, `[id]/edit`, `drafts`, `trash`, `published`, `responses`). `src/app/f/[slug]/` is the public respondent page (outside both groups). `src/app/auth/callback/route.ts` handles the Supabase code exchange.
- **Form editor**: `src/app/(app)/forms/[id]/edit/` is a multi-file Client Component editor (`form-editor.tsx` orchestrates `field-list`, `field-property-sheet`, `live-preview`, `sketch-reference-panel`, `publish-bar`, `publish-readiness`, `review-queue-banner`). It surfaces `needs_review` fields via `use-review-queue.ts`.
- **Data vs. API layers**: `src/lib/data/*` holds server-side Supabase reads (`forms`, `public-form`, `responses`); `src/lib/api/forms.ts` is the typed client for the FastAPI backend (`generateForm`, `pollGenerationJob`, `validateDefinition`, `waitForGenerationJob`). Mutations are Server Actions — `forms/actions.ts` (draft delete/restore), `responses/actions.ts`, and `(app)/tour-actions.ts` (`markTourCompleted`).
- **Product tour & theming**: `components/product-tour.tsx` drives the in-app tour (gated on `profiles.tour_completed_at`); `components/theme-provider.tsx` + `theme-toggle.tsx` provide light/dark mode (light is the default).
...
- `observability.py` — optional Langfuse tracing. Helpers no-op when `LANGFUSE_PUBLIC_KEY` / `LANGFUSE_SECRET_KEY` are unset (no exceptions, no log noise). `trace_root(...)` opens a root span; `get_langchain_callback_handler()` nests LLM calls under it; `flush()` is called after each job so traces aren't dropped when Lambda/background tasks freeze. `jobs.py` wraps `run_generation_job` in a `trace_root` span.
